...
export function scoreBusiness(input: {
  ...
  if (!passesRating) reasons.push(`rating ${rating} < ${config.minRating}`);
  if (!passesReviews) reasons.push(`reviews ${reviews} < ${config.minReviewCount}`);
  if (!passesWebsite) reasons.push(`business has a website — only businesses without websites qualify`);
  ...
  const reviewScore =
    clampedReviews <= 0
      ...
      : Math.round(
          Math.min(
            100,
            (Math.log(clampedReviews + 1) / Math.log(config.reviewCountCap + 1)) * 100,
          ),
        );
