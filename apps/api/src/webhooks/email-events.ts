...
/**
 * Maps Resend webhook event types to our EmailEventType enum.
 */
...
/**
 * Maps event types to the highest OutreachMessage status they should set.
 * Status only moves forward (opened > delivered > sent), never backwards.
 */
...
/**
 * Handles Resend email event webhooks (delivered, opened, clicked, bounced, complained).
 * Correlates events to outreach messages via the X-Outreach-Message-Id header or Resend email ID.
 */
export async function handleEmailEventsWebhook(
  req: Request,
  res: Response,
): Promise<void> {
  ...
  if (!eventType) {
    // Event type we don't track (e.g. email.sent) — acknowledge and move on
  ...
  // Try custom header first, then fall back to Resend email ID
  ...
  // ── Advance outreach message status if appropriate ──
